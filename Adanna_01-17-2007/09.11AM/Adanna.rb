...
class Adanna
	...
	#***********************************************************************
	def run_agent(strAgentFilename)
		retVal = nil
		...
		if File.exist?(strAgentFilename)
			@adannaLogger.log(PL.INFO, "Will launch agent script " + strAgentFilename + " in one minute", @loggingMode)
			...
			schtasksScript = "schtasks /Create /S " + @agentData.computerName + " /RU nathanl /RP ebjvtlqt /SC ONCE /TN Bot_AdvancedSearch /ST " + Util.strPad(2, time.hour.to_s, "0") + ":" + Util.strPad(2, time.min.to_s, "0") + ":" + Util.strPad(2, time.sec.to_s, "0") + " /TR \"ruby C:/Adanna/Agents/" + strAgentFilename
				...
				begin
						...
						if agent_registered?
							...
							if taskList.length == 0
								...
								if not (agentHash = get_agent_for_task(taskList[0])).nil? # Get the agent hash for the task hash
									...
									agentScript = run_agent(agentHash["script_name"])
								else
					...
					if agentScript.nil?
						...
						begin
							...
							puts "Broke out of while loop"
						rescue => ex
							@adannaLogger.log(PL.ERROR, "Exception was caught! Inner message: " + ex.message, @loggingMode)
							next
						end
