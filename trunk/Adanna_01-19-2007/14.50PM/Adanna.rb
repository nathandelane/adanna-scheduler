...
class Adanna
	...
	def send_script_history(id_script, id_host)
		...
		queryString = "INSERT INTO script_history(" +
			...
			"VALUES(" + id_script + ", 1, '" + id_host + "', 1, '" + @date_time + "', '" + @date_time + "', '" + @date_time + "', 'not', 0)"
	...
	#***********************************************************************
	# update_script_History updates the script history with end of script
	# data.
	#***********************************************************************
	def update_script_history(strEndTime, strTotalTime, id_script_history)
		# Create a query string
		# Get today's date and time and make a date time for Mysql
		date = Date.today
		strEndTime.chomp!
	...
	def update_properties_file(intPropertiesFile)
		if not intPropertiesFile == 0
			...
			if File.exist?(intPropertiesFile)
				File.unlink(intPropertiesFile)
			end # End: if File.exist?(strScriptName)
			# Build a query string
			queryString = "SELECT * FROM script_file WHERE id = " + intPropertiesFile
			# Query the db
			resultSet = @binding.query(queryString)
			...
			if resultSet.num_rows == 1
				...
				fw = File.new(strScriptName, "w+")
				fw.write(fileContents)
				fw.close
			end # End: if resultSet.num_rows == 1
			@adannaLogger.log(PL.INFO, "Properties file was updated successfully", @loginggMode)
		else
			@adannaLogger.log(PL.INFO, "Not updating properties file - script was not associated with a properties file", @logingMode)
	...
	def update_xml_environment_file(intXMLFile)
		if not intXMLFile == 0
			...
			# Delete the old file if it exists
			if File.exist?(intXMLFile)
				File.unlink(intXMLFile)
			end # End: if File.exist?(strScriptName)
			# Build a query string
			queryString = "SELECT * FROM script_file WHERE id = " + intXMLFile
			# Query the db
			resultSet = @binding.query(queryString)
			# Try and create the file
			if resultSet.num_rows == 1
			...
			@adannaLogger.log(PL.INFO, "Not updating XML environment file - script was not associated with an environment file", @logingMode)
				...
				begin
						...
						if agent_registered?
							...
							if taskList.length == 0
								...
								if not (@agentHash = get_agent_for_task(taskList[0])).nil? # Get the agent hash for the task hash
									...
									update_properties_file(taskList[0]["ID_script_file_properties"])
									# Update the XML environment file
									update_xml_environment_file(taskList[0]["ID_script_file_xml_environment"])
					...
					if agentScript.nil?
						...
						begin
							...
							update_script_history(agentEndTime, agentTotalTime, @scriptHistoryID)
