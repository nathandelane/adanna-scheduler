...
class Adanna
	...
	def initialize()
		...
		# Set version
		@version = "1.2.4" + "_" + Pod::libraryVersion?
	...
	def run_agent(strAgentFilename, strTime)
		...
		if File.exist?(strAgentFilename)
			...
			schtasksScript = "schtasks /Create /S " + @agentData.computerName + " /RU nathanl /RP ebjvtlqt /SC ONCE /TN " + taskName[0].to_s + " /ST " + strTime + " /TR \"ruby C:/Adanna/Agents/" + strAgentFilename
	...
	def update_adanna_script()
		...
		if resultSet.num_rows == 1
			...
			if not row["script_file_description"] == @version
				@adannaLogger.log(PL.INFO, "Current version=" + @version + " New version=" + row["script_file_description"], @loggingMode)
						...
						if agent_registered?
							...
							if taskList.length == 0
								...
								if not (@agentHash = get_agent_for_task(taskList[0])).nil? # Get the agent hash for the task hash
									...
									commandLine = @agentHash["script_name"] + " " + taskList[0]["scheduler_script_command_line"]
									@adannaLogger.log(PL.INFO, "Running task: " + commandLine, @loggingMode)
									agentScript = run_agent(commandLine, taskList[0]["scheduler_time"])
								else
