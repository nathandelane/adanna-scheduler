...
class Adanna
	...
	def initialize()
		...
		# Set version
		@version = "1.2.8" + "_" + Pod::libraryVersion?
	...
	def run_agent(strAgentFilename, strCommandline, strTime)
		...
		if File.exist?(strAgentFilename)
			...
			schtasksScript = "schtasks /Create /S " + @agentData.computerName + " /RU nathanl /RP ebjvtlqt /SC ONCE /TN " + taskName[0].to_s + " /ST " + strTime + " /TR c:\\Adanna\\Agents\\Run_Agent.bat"
	...
	def adanna()
		...
		taskList = nil
		agentScript = nil
		intUpdateCountdown = (60 * 24) # Check for an update every 24 hour(s)
