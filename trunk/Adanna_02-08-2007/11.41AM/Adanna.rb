...
class Adanna
	...
	def initialize()
		...
		# Set version
		@version = "1.3.1" + "_" + Pod::libraryVersion?
	...
	def bind_to_scheduler_database()
		...
		begin
			dbConnection = DBI.connect("DBI:" + @configTable["scheduler_database"][0]["db_connection_type"][0] + ":" + @configTable["scheduler_database"][0]["db_database_name"][0], @configTable["scheduler_database"][0]["db_user_name"][0], @configTable["scheduler_database"][0]["db_user_password"][0])
			retVal = dbConnection
		rescue => ex
	...
	def bind_to_results_database()
		...
		begin
			dbConnection = DBI.connect("DBI:" + @configTable["results_database"][0]["db_connection_type"][0] + ":" + @configTable["results_database"][0]["db_database_name"][0], @configTable["results_database"][0]["db_user_name"][0], @configTable["results_database"][0]["db_user_password"][0])
			retVal = dbConnection
		rescue => ex
	...
	def bind_to_file_database()
		...
		begin
			dbConnection = DBI.connect("DBI:" + @configTable["file_location"][0]["db_connection_type"][0] + ":" + @configTable["file_location"][0]["database_name"][0], @configTable["file_location"][0]["user_name"][0], @configTable["file_location"][0]["user_password"][0])
			retVal = dbConnection
		rescue => ex
	...
	def send_log(timeStamp, status, details)
		...
		logInsertString = "INSERT INTO script_history_log(ID_script_history, script_history_log_event_datetime_stamp, script_history_log_event_status, script_history_log_event_details) VALUES('" +
			...
			details.to_s + "')"
		# Query the database
		results = @resBinding.execute(logInsertString)
	...
	def send_script_history(id_script, id_host)
		...
		@resBinding.execute(queryString)
		...
		resultSet = @resBinding.execute(queryString)
	...
	def update_script_history_finished(strEndTime, strTotalTime, id_script_history)
		...
		results = @resBinding.execute(queryString)
	...
	def get_all_tasks(timeSpanInSeconds)
		...
		@adannaLogger.log(PL.INFO, "Getting tasks for host " + @agentData.computerName + " within " + timeSpanInSeconds.to_s + " minutes of " + strTime_now, @loggingMode)
		...
		# Query the database
		resultSet = @binding.execute(agentTasksQueryString)
		# Initialize array
		taskList = Array.new
		# Get each row
		rows = resultSet.fetch_all
	...
	def get_agent_for_task(taskHash)
		...
		if taskHash.instance_of?(Hash)
			if taskHash.has_key?("ID_script")
				...
				resultSet = @fileBinding.execute(queryString)
				# Check result set
				rows = resultSet.fetch_all
				if not rows.size == 1
					retVal = nil
	...
	#***********************************************************************
	def run_agent(strAgentFilename, strCommandline, strTime)
		retVal = nil
		...
		if File.exist?(strAgentFilename)
			...
			batchString = "@echo off\n" +
				"C:\n" +
				"cd " + @adannaBase + "/" + @adannaAgentFolder + "\n" +
			...
			schtasksScript = "schtasks /Create /S " + @agentData.computerName + " /RU " + @configTable["adanna"][0]["task_run_user"][0] + " /RP " + @configTable["adanna"][0]["task_run_password"][0] + " /SC ONCE /TN " + taskName[0].to_s + " /ST " + strTime + " /TR c:\\Adanna\\Agents\\Run_Agent.bat"
	...
	def agent_registered?()
		...
		# Query the database
		resultSet = @binding.execute(agentRegistrationQueryString)
		# Vaildate and get ID
		while row = resultSet.fetch_hash do
			rows = resultSet.fetch_all
			if row["host_name"] == @agentData.computerName and rows.size == 1
				@adannaID = row["id"]
				retVal = true
			else
				retVal = false
	...
	def adanna()
		...
		if (not (@binding = bind_to_scheduler_database).nil?) and (not (@resBinding = bind_to_results_database).nil?) and (not (@fileBinding = bind_to_file_database).nil?) # Connect to the databases
			while true
				begin
					if check
						if agent_registered?
