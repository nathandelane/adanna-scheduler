...
class Adanna
	...
	def initialize()
		...
		# Set version
		@version = "1.3.0" + "_" + Pod::libraryVersion?
	...
	def bind_to_scheduler_database()
		...
		begin
			dbConnection = Mysql.real_connect(@configTable["scheduler_database"][0]["db_host_name"][0], @configTable["scheduler_database"][0]["db_user_name"][0], @configTable["scheduler_database"][0]["db_user_password"][0], @configTable["scheduler_database"][0]["db_database_name"][0])
			retVal = dbConnection
		rescue => ex
			retVal = nil
		end
	...
	def bind_to_results_database()
		...
		begin
			dbConnection = Mysql.real_connect(@configTable["results_database"][0]["db_host_name"][0], @configTable["results_database"][0]["db_user_name"][0], @configTable["results_database"][0]["db_user_password"][0], @configTable["results_database"][0]["db_database_name"][0])
			retVal = dbConnection
		rescue => ex
			retVal = nil
		end
	...
	def bind_to_file_database()
		...
		begin
			dbConnection = Mysql.real_connect(@configTable["file_location"][0]["host_name"][0], @configTable["file_location"][0]["user_name"][0], @configTable["file_location"][0]["user_password"][0], @configTable["file_location"][0]["database_name"][0])
			retVal = dbConnection
		rescue => ex
			retVal = nil
		end
	...
	def send_log(timeStamp, status, details)
		...
		logInsertString = "INSERT INTO script_history_log(ID_script_history, script_history_log_event_datetime_stamp, script_history_log_event_status, script_history_log_event_details) VALUES('" +
			...
			details.to_s + "')"
		# Query the database
		@resBinding.query(logInsertString)
	...
	def send_script_history(id_script, id_host)
		...
		resultSet = @resBinding.query(queryString)
	...
	def get_agent_for_task(taskHash)
		...
		if taskHash.instance_of?(Hash)
			if taskHash.has_key?("ID_script")
				...
				resultSet = @fileBinding.query(queryString)
				# Check result set
