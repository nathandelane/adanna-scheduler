...
class Adanna
	...
	def agent_registered?()
		@adannaLogger.log(PL.INFO, "Querying database to find out if ADAnna on host " + @agentData.computerName + " is registered", @loggingMode)
	...
	#***********************************************************************
	def run_agent_script(strScriptName)
	...
	def get_next_task(arrTaskList)
		...
		arrTaskList.each {
			...
			if taskTime > Time.now and taskTime < (Time.now + (5 * 60))
				taskHash = tempTask
